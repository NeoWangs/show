...
(function(win) {
	...
	win.setcookie = function(cookieName, cookieValue, config) {
		...
		if (typeof opt.expires === 'number') {
			var ltime = new Date();
			ltime.setDate(ltime.getDate() + opt.expires);
			opt.expires = ltime.toGMTString();
		}
		
		if(!cookieValue) cookieValue = "";
		var setStr = escape(cookieName) + '=' + escape(cookieValue) + ';path=' + opt.path;
		if(opt.expires){
			setStr += (';expires=' + opt.expires);
		}
		if(opt.domain){
			setStr += (';domain=' + opt.domain);
		}
		if(opt.secure) {
			setStr += "secure";
		}
